<article class="task_queue" id="user_task_queue">
        ...
        <section class="tab-content tab-content-scroll">
        	<div class="tab-pane active" id="queuedtasks">
        		<?php 
			 $multi_search = array();
			 $multi_search[0] =array('agentlist'=>$agentid, 'status' => 'ready');	
			 $multi_search[1]= array('agentlist'=>$agentid, 'status' => 'queued');	
			 $multi_search[2] = array('privacy'=>'public', 'status' => 'ready');	
			 $multi_search[3]= array('privacy'=>'public', 'status' => 'queued');
            ...
            <div class="tab-pane" id="ongoing">
          		<?php 
			 $multi_search = array();
			 $multi_search[0] = array('agentlist'=>$agentid, 'status' => 'started');	
			 $tasks = Task::find_by_multiarray_page_and_number($multi_search);
				include "snippet-tasks-table.php"; ?>
            </div> 
            <div class="tab-pane" id="completedtasks">
          		<?php 
			 $multi_search = array();
			 $multi_search[0] =array('agentlist'=>$agentid, 'status' => 'completed');	
			 $tasks = Task::find_by_multiarray_page_and_number($multi_search);
				include "snippet-tasks-table.php"; ?> 
            </div>
        </section><!-- tab-content -->
